import argparse, fileinput, os, re

parser = argparse.ArgumentParser(description='Migrate devtools localized strings.')
parser.add_argument('path', type=str, help='path of your L10N clone')
parser.add_argument('-c', '--config', type=str, help='path of the configuration file', default='config')
...
def replace_value_properties(path, name, value):
  ...
  if not found:
    f = open(path, 'a')
    f.write('\n# LOCALIZATION NOTE (' + name + '): LOCALIZATION NOTE MISSING\n')
    f.write(name + '=' + value + '\n')
    f.close()
...
for i, line in enumerate(lines):
  ...
  properties_path = os.path.join(args.path, 'devtools/client/', propertiesFile);
